...
public class MessageResonance extends javax.ws.rs.core.Application {
	...
	public Response getMessageResonance(@FormParam("message") String message)
			throws Exception {
		...
		if (VCAP_SERVICES != null
				&& ((JSONObject) JSON.parse(VCAP_SERVICES))
						.containsKey(Service_Name)) {
			...
			try {
				...
				ringScoreURI = endpointBaseURI + "/" + ringObject.getJSONObject("ringscore").getString(
						"path");
				datasetURI = endpointBaseURI + "/" + ringObject.getJSONObject("datasets").getString(
						"path");
		...
		if (message == null || message.isEmpty()) {
			...
			for (String word : words) {
				...
				HttpGet httpGet = new HttpGet(ringScoreURI + "?dataset=1&text="
				...
				boolean haveError = serviceResponseJson.containsKey("error")
						|| !serviceResponseJson.containsKey("overall");
				if (haveError) {
					System.out.println("Error detected in service response ----------------  " );
					System.out.println(serviceResponseString);
					resultString = new StringBuilder(
							"{\"error\": \"Service Error on " + word + "\"}");
					...
					int score = (int) serviceResponseJson.get("overall");
	...
	private String calculateColorCode(int scoreInt) {
		if (scoreInt < 11)
			return "black";
		else if (scoreInt < 21)
