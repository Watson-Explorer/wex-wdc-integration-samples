...
function d3_svg_singleArcEndAngle(d) {
  ...
  var d3_svg_brushResizes = [ [ "n", "e", "s", "w", "nw", "ne", "se", "sw" ], [ "e", "w" ], [ "n", "s" ], ["se", "sw"],[] ];
  
 d3.svg.sectorbrush = function() {
  var event = d3_eventDispatch(sectorbrush, "brushstart", "brush", "brushend"),
      angleScale=null, //angle scale
      radiusScale=null, //radius scale
      
      minAngle=null,//min angle
      maxAngle=null,//max angle
      innerR=null, //inner radius
      outerR=null,//outer radius
      startAngle=null,//start angle
      endAngle=null,//end angle
      initDragStartAngle=null, //start angle when drag is initalized
      initDragEndAngle=null, //end angle when drag is initalized
      resizes = d3_svg_brushResizes[3]; // the extent in data space, lazily created

          

  function sectorbrush(g) {
    g.each(function() {
      var g = d3.select(this),
     ...
     if(angleScale &&radiusScale){
      ...
      fg.enter().append("path")
           .attr("d",arcfg)
          .attr("class", "extent")
          .style("visibility", sectorbrush.empty() ? "hidden" : "visible")
          .style("cursor", "move");
      ...
      tz.style("display", sectorbrush.empty() ? "none" : null);

      // Remove any superfluous resizers.
      tz.exit().remove();

   
   //redraw the extent area
   if(angleScale && radiusScale &&  startAngle &&  endAngle)
        redrawExtent(g);
       
      
      
      //move the resize bars to new position
      redrawResizers(g);
    });
  }

  function redrawResizers(g) {
   var resizebar = d3.svg.arc()
             .startAngle(function(d) { if (/e$/.test(d)) return endAngle; else return startAngle; })
             .endAngle(function(d) { if (/e$/.test(d)) return endAngle+1.3/innerR; else return startAngle+1.3/innerR; })
             .innerRadius(innerR)
             .outerRadius(outerR); 
   
    g.selectAll(".resize > path") .attr("d", resizebar);
  }

  function redrawExtent(g) {
       var newfg=d3.svg.arc()
         .startAngle(startAngle)
          .endAngle(endAngle)
          .innerRadius(innerR)
          .outerRadius(outerR);
                    
    g.select(".extent").attr("d", newfg);
  }


   function pointToAngle(p){
      //the point p is translated into a local coordinate.
      //for example, angle of p(1,-1) is the angle between orgin (0,0) and p,
      //so it is PI/4;
      if(p){
         ...
         if(-p[0]/p[1]>0) theta=Math.atan(-p[0]/p[1]);
         else theta=Math.PI+Math.atan(-p[0]/p[1]);
   ...
   function brushstart() {
    var target = this,
        ...
        originAngle = pointToAngle(mouse()),
        initalBrush=true;
    ...
    // store the angles when the drag starts.
    if (dragging) {
      
      initDragStartAngle=startAngle;
      initDragEndAngle=endAngle;
    }

    // If a resizer was clicked on, record which side is to be resized.
    else if ((resizing=="sw" || resizing=="se")&&angleScale) {
      //TODO hard-coded, the issue is when no resize action, there might be some data in this field. 
      
      if(/w$/.test(resizing))
         originAngle = endAngle;
         else originAngle = startAngle;
      
    }

    // Propagate the active cursor to the body for the drag duration.
    g.style("pointer-events", "none").selectAll(".resize").style("display", null);
    g.select(".extent").style("visibility", "visible");
    //g.select(".background").style("visibility", "visible");
    d3.select("body").style("cursor", eventTarget.style("cursor"));
    ...
    function brushmove() {
      ...
      if (angleScale && move1(pointToAngle(point))) {       
        redrawExtent(g);
        moved = true;
      }
  ...
  function move1(curAngle){
   
   //TODO: the boundary still has a bug when after switch the start and end resizers. 
       startAngle=originAngle;
        ...
        endAngle=curAngle;
         ...
         // When dragging, record the offset of angle.
      if (dragging) {
         
         var dAngle=curAngle-originAngle
            extAngle=initDragEndAngle-initDragStartAngle;
            
         startAngle=initDragStartAngle+dAngle;
         endAngle=initDragEndAngle+dAngle;
         
         if(endAngle>maxAngle || startAngle>maxAngle) {
            endAngle=maxAngle;
            startAngle=endAngle-extAngle;
            }
         if (startAngle<minAngle || endAngle<minAngle) {
            startAngle=minAngle;
            endAngle=startAngle+extAngle;
            }
    ...
    function brushend() {
      ...
      g.style("pointer-events", "all").selectAll(".resize").style("display", sectorbrush.empty() ? "none" : null);
      g.select(".extent").style("visibility",sectorbrush.empty() ? "hidden" : "visible");
      //g.select(".background").style("visibility",sectorbrush.empty() ? "hidden" : "visible");
      d3.select("body").style("cursor", null);
  ...
  sectorbrush.angleScale = function(z) {
    if (!arguments.length) return angleScale;
    angleScale = z;
    var d=angleScale.domain();
      minAngle=d[0];//start angle
      maxAngle=d[1];//start angle

    resizes = d3_svg_brushResizes[3];//hardcoded! TODO:adjust dynamically 
    return sectorbrush;
  };

  sectorbrush.radiusScale = function(z) {
    if (!arguments.length) return radiusScale;
    radiusScale = z;
    var d=radiusScale.domain();     
      innerR=d[0]; //inner radius
      outerR=d[1];//outer radius
    resizes = d3_svg_brushResizes[3]; //hardcoded! TODO:adjust dynamically 
    return sectorbrush;
  };


  sectorbrush.extent = function(z) {
  ...
  sectorbrush.empty = function() {
    return (angleScale && startAngle===endAngle);
  };
    
  return d3.rebind(sectorbrush, event, "on");
};
